...
I<list> followed by a directory path, that is a group name, lists all
entries inside the specified group as well as all its subgroups. If not
name is given, all entries will be listed. Passing I<-g> as first
parameter causes it to list groups only.

I<help> prints information about how to use spm.
